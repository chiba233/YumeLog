...
- **数据处理**: `js-yaml` (仅用于解析非富文本内容，自制递归DSL语法不是很适配yaml所以用了特殊替代，DOM 渲染由自研 Parser 全权接管)
...
yumeLog 采用块级架构与自研 DSL 引擎渲染博客文本。文章可以指定 `id` 以便路由访问，也可以直接使用 `文章标题` 作为页面链接，但是这两个你肯定必须写一个。
...
- **注意:** RAW BLOCK为了防止意外闭合，必须独占一行且不允许空格。
...
$$bold(hello world)$$
```
解析器会把整个内容视为一个参数，不进行任何拆分。

### 2. 多参数 DSL
当 DSL 定义多个参数时，使用 `|` 分隔：
```text
$$link([https://google.com](https://google.com) | Google)$$
```
解析结果为 `url = "https://google.com"`, `text = "Google"`。
...
```
`text` 会 fallback 为 `url`。最终渲染效果：`<a href="https://google.com">https://google.com</a>`
...
```
解析结果：`title = "title"`, `text = "textextramore"`。

### 5. `|` 分隔符解析规则
`|` 只会在 **纯文本 token** 中被识别。这意味着：
```text
...
$$info(|hello)$$
```
解析结果：`title = ""`, `text = "hello"`。是否允许空值取决于 DSL 类型本身。
...
为了让 Parser 能够精确处理你的文本逻辑，引擎引入了**双层脱壳转义管线**。当你想在内容中输出语法关键字本身时，必须使用反斜杠 `\`。

### 1. 保护 DSL 闭合标记 `)`
如果你想在参数内使用 `)` 并且不希望它提前触发 `)$$` 闭合指令，可以使用 `\)`：
```text
$$warning(报错啦 | 此函数的结尾包含一个转义的括号： \)$$ )$$
```
渲染结果：此函数的结尾包含一个转义的括号： `)`

### 2. 保护管道符 `|`
管道符 `|` 是多参数 DSL 的天然切割器。如果你的文章标题里正好包含了 `|`，你需要转义它 `\|`，否则你的标题会被腰斩：
```text
...
### 3. 渲染真实的反斜杠 `\`
在极其罕见的代码演示场景下，如果你需要向用户展示连串的反斜杠（例如文件路径），你需要使用双反斜杠 `\\` 来代表一个实际显示的 `\`：
```text
$$info(路径说明 | 请访问 C:\\\\Users\\\\Admin 文件夹)$$
```
渲染结果：请访问 `C:\Users\Admin` 文件夹。

### 4. 保护块级结构 `@`
在 `@text` 或 `@image` 这样的块级架构中，如果行首正好需要输出 `@type` 的文本，会导致结构崩溃。此时可以在行首使用 `\` 保护它：
```text
@text
...
@end
```
*(注：`parseRichText` 引擎内部搭载了递归的 `unescapeAST`，所以无需担心转义符污染最终 DOM 结构。)*
...
专为图片高可用设计，支持定义主地址与热备地址。由于这部分是由 YAML 解析，请遵循标准的 YAML 缩进规范：
`- `为创建一个object组, `  `两个空格为为上一个object组添加对象，`    `四个空格为多行元素缩进。
...
introduction:
  - type: "zh"
    content: |
      这是基础信息，你可以访问/public/main/introduction.yaml进行修改
  - type: "en"
    content: |
