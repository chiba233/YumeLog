...
import {
  ...
  yamlLoading,
  yamlRetrying,
} from "@/shared/lib/yaml";
import { formatDate, formatTime, lang } from "@/shared/lib/app/setupLang.ts";
...
import { getSlug, useRouteModal } from "@/shared/lib/app/useRouteModal.ts";
import RichTextRenderer from "@/features/blog/components/RichTextRenderer.vue";
...
const renderDetailContent = (): VNodeChild => {
  ...
  return h("div", { class: "postCardMain" }, [
    ...
    blocks.map((block: PostBlock) => {
      ...
      const isLazyText = block.type === "text" && true;
