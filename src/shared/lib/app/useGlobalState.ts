...
import { parseRichText, stripRichText } from "@/shared/lib/dsl/BlogRichText/blogFormat.ts";
import friendsMessage from "@/data/I18N/friendsMessage.json";
import { socialRawData } from "@/shared/lib/app/setupJson.ts";
...
export const getDescriptionText = (blocks?: PostBlock[], targetLength = 160): string => {
  return getDescriptionTextWithStripper(blocks, targetLength, stripRichText);
};

export const getDescriptionTextWithStripper = (
  blocks: PostBlock[] | undefined,
  targetLength: number,
  stripper: (content: string) => string,
): string => {
  ...
  for (const block of blocks) {
    if (block.type === "text") {
      ...
      if (content.trim().length > 0) {
        const strippedBlock = stripper(content);
